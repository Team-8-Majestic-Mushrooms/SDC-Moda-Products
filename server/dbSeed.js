...
const configs = [
  ...
  },
  {
    ...
    query: 'INSERT INTO feature (id, product_id, feature, value) VALUES ($1, $2, $3, $4)',
  },
  {
    tableName: 'related',
    // filePath: "./seeds/testRelated.csv",
    filePath: './seeds/related.csv',
    query: 'INSERT INTO related (id, current_product_id, related_product_id) VALUES ($1, $2, $3)',
  },
  {
    tableName: 'style',
  ...
  },
  {
    ...
    query: 'INSERT INTO sku (id, style_id, size, quantity) VALUES ($1, $2, $3, $4)',
  },
  {
    tableName: 'photo',
    // filePath: './seeds/testPhotos.csv',
    filePath: './seeds/photos.csv',
    query: 'INSERT INTO photo (id, style_id, url, thumbnail_url) VALUES ($1, $2, $3, $4)',
  },
];

const seedDatabase = ({ tableName, filePath, query }, client) =>
    ...
    try {
      fs.createReadStream(filePath)
        ...
        .on('data', (row) => {
          ...
          client.query(query, treatedRow).then(() => {});
        })
        .on('error', (err) => {
...
const main = async () => {
  ...
  await seedDatabase(configs[1], client);
